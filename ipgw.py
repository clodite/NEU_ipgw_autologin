...
def timed_prompt(prompt: str, timeout: int, default_value, allowed_keys: list = None):
    ...
    if result[0] is None:
        print(f"\r{prompt}超时，默认选择 [{default_value}]", flush=True)
        return default_value

    # 用户输入后，清理行并输出用户选择
    print(f"\r\n{prompt}你选择了 [{result[0]}]", flush=True)
    return result[0]
...
with sync_playwright() as p:
    ...
    chromium_path = get_resource_path(os.path.join("chromium", chromium_executable))
